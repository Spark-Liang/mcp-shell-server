...
async def run_http_server(
    host: str, port: int, path: str, web_path: Optional[str] = None
) -> None:
    ...
    try:
        ...
        async with streamable_http_server(host=host, port=port, path=path) as (
            read_stream,
            write_stream,
        ):
            await app.run(
    ...
    def run_web_server():
        ...
        try:
            web_server.start_web_interface(
                host=host, port=port, debug=debug, url_prefix=url_prefix
            )
        except Exception as e:
...
def stdio(web_host, web_port, web_path):
    """使用stdio模式启动服务器（默认模式）"""
    asyncio.run(
        run_stdio_server(web_host=web_host, web_port=web_port, web_path=web_path)
    )
