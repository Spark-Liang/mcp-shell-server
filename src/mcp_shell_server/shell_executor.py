...
from typing import IO, Any, Dict, Iterable, List, Optional, Set, Union
...
from .env_name_const import COMSPEC, DEFAULT_ENCODING, SHELL
...
class ShellCommandResponse(BaseModel):
    ...
    status: int = Field(
        ..., description="Command exit code (0=success, non-zero=error, -1=timeout)"
    )
    ...
    directory: Optional[str] = Field(
        None, description="Directory where the command was executed"
    )
