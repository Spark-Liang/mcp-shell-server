    ...
    try:
        ...
        try:
            addresses = socket.getaddrinfo(
                host=hostname, port=None, family=socket.AF_INET  # 只获取IPv4地址
            )
...
def _format_to_number(
    mix_format_number: Optional[Union[int, float, str]],
    ...
    if isinstance(mix_format_number, str):
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid value: {mix_format_number}, must be an string in format of integer or a float"
                )
...
async def shell_execute(
    command: List[str] = Field(description="Command and its arguments as array"),
    directory: str = Field(
    ...
    envs: Optional[Dict[str, str]] = Field(
        default=None, description="Additional environment variables for the command"
    ),
    encoding: str = Field(
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f"Command execution timed out after {timeout or DEFAULT_TIMEOUT} seconds"
            )
        ...
        content = []
        content.append(
            TextContent(type="text", text=f"**exit with {result.get('status')}**")
        )
...
async def shell_bg_start(
    ...
    description: str = Field(description="Description of the command (required)"),
    labels: Optional[List[str]] = Field(
        default=None, description="Labels to categorize the command"
    ),
    stdin: Optional[str] = Field(
        default=None,
        description="Input to be passed to the command via stdin",
    ...
    try:
        ...
        return [
            TextContent(type="text", text=f"Started background process with ID: {pid}")
        ]
    except Exception as e:
...
async def shell_bg_list(
    labels: Optional[List[str]] = Field(
        default=None, description="Filter processes by labels"
    ),
    status: Optional[str] = Field(
        default=None,
        description="Filter processes by status ({})".format(
            ", ".join([f"'{s.value}'" for s in ProcessStatus])
        ),
    ),
) -> Sequence[TextContent]:
    ...
    try:
        ...
        if status and status not in [s.value for s in ProcessStatus]:
            raise ValueError(
                f"Status must be one of: {', '.join([s.value for s in ProcessStatus])}"
            )
        ...
        processes = await default_shell_executor.list_processes(
            labels=labels, status=status_enum
        )

        if not processes:
            return [TextContent(type="text", text="No background processes found")]
        ...
        for proc in processes:
            ...
            pid_short = (
                proc.shell_cmd.split()[0][:8]
                if len(proc.shell_cmd.split()) > 0
                else "N/A"
            )
            ...
            lines.append(
                f"{pid_short} | {proc.status.value} | {start_time_str} | {cmd_str} | {proc.description} | {labels_str}"
            )
...
async def shell_bg_stop(
    pid: int = Field(description="ID of the process to stop"),
    force: bool = Field(default=False, description="Whether to force stop the process"),
) -> Sequence[TextContent]:
    ...
    try:
        ...
        cmd_str = (
            ...
            else process.process_info.shell_cmd
        )
        if len(cmd_str) > 30:
            cmd_str = cmd_str[:27] + "..."
        ...
        description = (
            ...
            else process.process_info.description
        )
...
async def shell_bg_logs(
    pid: int = Field(description="ID of the process to get output from"),
    ...
    with_stdout: bool = Field(default=True, description="Show standard output"),
    with_stderr: bool = Field(default=False, description="Show error output"),
    add_time_prefix: bool = Field(
        default=True, description="Add timestamp prefix to each output line"
    ),
    ...
    if follow_seconds is not None and follow_seconds < 0:
        raise ValueError(
            f"Invalid follow_seconds value: {follow_seconds}, must be a non-negative number"
        )
    ...
    try:
        ...
        # 获取进程信息
        status_value = process.status.value if hasattr(process, "status") else "unknown"
        ...
        if with_stdout:
            ...
            initial_stdout_output = await default_shell_executor.get_process_output(
                pid=pid, tail=tail, since=since, until=until, error=False
            )

        if with_stderr:
            # 使用 ShellExecutor 获取错误输出
            initial_stderr_output = await default_shell_executor.get_process_output(
                pid=pid, tail=tail, since=since, until=until, error=True
            )
        ...
        if with_stdout:
            if stdout_output:
                ...
                result_content.append(
                    TextContent(type="text", text="No standard output available")
                )
        ...
        if with_stderr:
            if stderr_output:
                ...
                result_content.append(
                    TextContent(type="text", text="No error output available")
                )

        # 添加帮助信息
        extra_info_lines = ["---", "extra infos:", "---", "**Follow Options:**"]

        if follow_seconds > 0:
            extra_info_lines.append(
                f"- Showing logs with {follow_seconds}s follow time"
            )
            extra_info_lines.append(
                f"- For longer follow: `shell_bg_logs(pid={pid}, follow_seconds=60"
            ...
            extra_info_lines.append(
                f"- To follow logs: `shell_bg_logs(pid={pid}, follow_seconds=5"
                + (", with_stderr=True" if with_stderr else "")
                + ")`"
            )
...
async def shell_bg_clean(
    pids: List[int] = Field(description="要清理的进程ID列表"),
) -> Sequence[TextContent]:
    """Clean background processes that have completed or failed"""

    # 结果表格
    result_table = {"cleaned": [], "failed": [], "running": []}
        ...
        try:
            ...
            if not process:
                result_table["failed"].append(
                    {"pid": pid, "reason": "Process not found"}
                )
                continue
            ...
            if is_running:
                result_table["running"].append(
                    {
                        "pid": pid,
                        "command": getattr(process, "command", "Unknown command")[:30]
                        + ("..." if len(getattr(process, "command", "")) > 30 else ""),
                    }
                )
                continue
            ...
            result_table["failed"].append({"pid": pid, "reason": str(e)})
    ...
    if result_table["cleaned"]:
        result_lines.append(
            f"**Successfully cleaned {len(result_table['cleaned'])} processes:**"
        )
...
async def shell_bg_detail(
    pid: int = Field(description="ID of the process to get details for"),
) -> Sequence[TextContent]:
    ...
    try:
        ...
        lines.append(
            f"To view error output: `shell_bg_logs(process_id={pid}, with_stderr=True)`"
        )
        ...
        # 根据进程状态提供不同的命令
        if status == ProcessStatus.RUNNING or (
            isinstance(status, str) and status == "running"
        ):
            lines.append(f"Stop the process: `shell_bg_stop(process_id={pid})`")
            lines.append(
                f"Force stop the process: `shell_bg_stop(process_id={pid}, force=True)`"
            )
        else:
...
def start_web_server(
    ...
    def run_web_server():
        """在线程中运行Web服务器"""
        try:
            web_server.start_web_interface(
                host=host, port=port, debug=debug, prefix=url_prefix
            )
        except Exception as e:
...
@click.option(
    "--web-port", default=None, type=int, help="Web服务器端口，不指定则使用随机端口"
)
...
@click.option(
    "--web-path", default="/web", help="Web服务器路径，不指定则与SSE服务器共用同一端口"
)
...
@click.option(
    "--web-path", default="/web", help="Web服务器路径，不指定则与HTTP服务器共用同一端口"
)
def http(host, port, path, web_path):
    """使用streamable HTTP模式启动服务器"""
    logger.info(
        f"Starting MCP shell server (HTTP mode) v{__version__} on {host}:{port}{path}"
    )
