...
from .interfaces import LogEntry, ToolHandler
...
class StartBackgroundProcessToolHandler(ToolHandler[StartProcessArgs]):
    ...
    async def _do_run_tool(self, arguments: StartProcessArgs) -> Sequence[TextContent]:
        ...
        try:
            ...
            return [
                TextContent(
                    type="text", text=f"Started background process with ID: {pid}"
                )
            ]
...
class ListProcessesArgs(BaseModel):
    ...
    def validate_status(cls, v):
        if v and v not in [status.value for status in ProcessStatus]:
            raise ValueError(
                f"Status must be one of: {', '.join([status.value for status in ProcessStatus])}"
            )
        return v
    ...
    def validate_timestamps(cls, values):
        if isinstance(values, dict):
            ...
            if (
                "since" in values
                and values["since"]
                and isinstance(values["since"], str)
            ):
                try:
                    values["since"] = datetime.fromisoformat(values["since"])
                except ValueError:
                    raise ValueError(
                        "'since' must be a valid ISO format datetime string (e.g. '2021-01-01T00:00:00')"
                    )
            ...
            if (
                "until" in values
                and values["until"]
                and isinstance(values["until"], str)
            ):
                try:
                    values["until"] = datetime.fromisoformat(values["until"])
                except ValueError:
                    raise ValueError(
                        "'until' must be a valid ISO format datetime string (e.g. '2021-01-01T00:00:00')"
                    )
        return values
...
class GetBackgroundProcessOutputToolHandler(ToolHandler[GetProcessOutputArgs]):
    ...
    async def _do_run_tool(
        self, arguments: GetProcessOutputArgs
        ...
        try:
            ...
            # 获取进程信息
            status_value = (
                process.status.value if hasattr(process, "status") else "unknown"
            )
            ...
            if with_stdout:
                ...
                if stdout_output:
                    ...
                    stdout_content = self._format_process_output(
                        stdout_output, "STDOUT", add_time_prefix, time_prefix_format
                    )
            ...
            if with_stderr:
                ...
                if stderr_output:
                    ...
                    stderr_content = self._format_process_output(
                        stderr_output, "STDERR", add_time_prefix, time_prefix_format
                    )
            ...
            if follow_seconds is not None:
                help_lines = [
                    ...
                    "- For standard output: `shell_bg_logs(pid={}, follow_seconds=60)`".format(
                        pid
                    ),
                ]
                if with_stderr:
                    help_lines.append(
                        "- For error output: `shell_bg_logs(pid={}, with_stderr=True, follow_seconds=60)`".format(
                            pid
                        )
                    )
...
class CleanBackgroundProcessToolHandler(ToolHandler[CleanProcessArgs]):
    ...
    async def _do_run_tool(self, arguments: CleanProcessArgs) -> Sequence[TextContent]:
        ...
        if not pids:
            return [
                TextContent(type="text", text="No process IDs provided to clean up")
            ]
            ...
            try:
                ...
                if is_running:
                    result_table["running"].append(
                        {
                            "pid": pid,
                            "command": getattr(process, "command", "Unknown command")[
                                :30
                            ]
                            + (
                                "..."
                                if len(getattr(process, "command", "")) > 30
                                else ""
                            ),
                        }
                    )
        ...
        if result_table["cleaned"]:
            ...
            for proc in result_table["cleaned"]:
                result_lines.append(
                    f"- PID: {proc['pid']} | Command: {proc['command']}"
                )
...
class GetBackgroundProcessDetailToolHandler(ToolHandler[GetProcessDetailArgs]):
    ...
    async def _do_run_tool(
        self, arguments: GetProcessDetailArgs
    ) -> Sequence[TextContent]:
        pid = arguments.pid
        ...
        try:
            ...
            lines.append(
                "To view error output: `shell_bg_logs(pid={}, with_stderr=True)`".format(
                    pid
                )
            )
            ...
            if status == ProcessStatus.RUNNING or (
                ...
                lines.append(
                    "Force stop the process: `shell_bg_stop(pid={}, force=True)`".format(
                        pid
                    )
                )
            else:
                lines.append(
                    "Clean up the process: `shell_bg_clean(pids=[{}])`".format(pid)
                )
