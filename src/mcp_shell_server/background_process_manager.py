...
class BackgroundProcess(ExtendedProcess):
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        raise AttributeError(
            f"'BackgroundProcess' object has no attribute '{name}' and internal process is None"
        )
    ...
    async def communicate(
                ...
                if timeout is not None:
                    ...
                    try:
                        return await asyncio.wait_for(
                            self.process.communicate(input=input), timeout=timeout
                        )
        ...
        if self._stdout_logger and self._stderr_logger:
            ...
            # 将日志记录对象转换为字节字符串
            stdout_bytes = "\n".join([log.text for log in stdout_logs]).encode(
                self.encoding
            )
            stderr_bytes = "\n".join([log.text for log in stderr_logs]).encode(
                self.encoding
            )
...
class BackgroundProcessManager(IProcessManager):
    ...
    def __init__(self):
        ...
        # 进程保留时间设置（秒）
        self._auto_cleanup_age = int(
            os.environ.get(PROCESS_RETENTION_SECONDS, 300)
        )  # 默认5分钟
            ...
            if self._processes:
                for pid, bg_process in list(self._processes.items()):
                    try:
                        ...
                        logger.warning(f"终止进程时出错 (信号 {signum}): {e}")
            ...
            while True:
                ...
                try:
                    decoded_line = line.decode(
                        bg_process.encoding, errors="replace"
                    ).rstrip()
                    ...
                    if (
                        ...
                        or (current_time - last_flush_time) >= buffer_timeout
                    ):
                        if is_error:
                            # 批量添加错误输出
                            if callable(
                                getattr(bg_process._stderr_logger, "add_lines", None)
                            ):
                            ...
                            # 批量添加标准输出
                            if callable(
                                getattr(bg_process._stdout_logger, "add_lines", None)
                            ):
                    ...
                    if bg_process.timeout is not None:
                        ...
                        try:
                            exit_code = await asyncio.wait_for(
                                bg_process.process.wait(), timeout=bg_process.timeout
                            )
                            ...
                            # 超时发生，记录信息并终止进程
                            logger.warning(
                                f"进程 {bg_process.pid} 执行超时 ({bg_process.timeout}秒)"
                            )
                            bg_process.add_error(
                                f"进程执行超时，超过 {bg_process.timeout} 秒"
                            )
                            ...
                            try:
                                ...
                                try:
                                    await asyncio.wait_for(
                                        bg_process.process.wait(), timeout=2.0
                                    )
                                except asyncio.TimeoutError:
                                    # 如果仍然无法终止，强制结束
                                    if asyncio.iscoroutinefunction(
    ...
    async def create_process(
        ...
        try:
            ...
            raise ValueError(f"进程创建过程中出现意外错误: {str(e)}") from e
    ...
    async def get_process_output(
        ...
        try:
            if error:
                return bg_process.get_error(
                    tail=tail, since=since_time, until=until_time
                )
            else:
                return bg_process.get_output(
                    tail=tail, since=since_time, until=until_time
                )
        except Exception as e:
    ...
    async def cleanup_processes(
        self,
        processes: Optional[
            List[Union[asyncio.subprocess.Process, BackgroundProcess]]
        ] = None,
        labels: Optional[List[str]] = None,
                        ...
                        if proc.is_running():
                            ...
                            if proc_pid and proc_pid in self._processes:
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"等待 BackgroundProcess 终止时出错: {e}"
                                    )
                                count += 1
            ...
            for i, cmd in enumerate(commands):
                ...
                try:
                    stdout, stderr = await self.execute_with_timeout(
                        process, stdin=prev_stdout, timeout=timeout
                    )
