...
async def test_create_process(bg_process_manager, cleanup_bg_processes):
    ...
    with (
        ...
        patch.object(bg_process_manager, "_monitor_process", AsyncMock()),
    ):
...
async def test_cleanup_all(bg_process_manager):
    ...
    # 添加到进程管理器
    bg_process_manager._processes = {1001: process1, 1002: process2}
...
async def test_cleanup_processes(bg_process_manager):
    ...
    # 准备清理方法的mock
    with patch.object(
        bg_process_manager, "cleanup_process", AsyncMock()
    ) as mock_cleanup:
        # 测试按状态过滤 - 应该清理 COMPLETED 状态的进程
        # 重置进程字典
        bg_process_manager._processes = {1001: process1, 1002: process2, 1003: process3}
...
async def test_process_timeout(bg_process_manager, cleanup_bg_processes):
    ...
    with (
        ...
        patch("asyncio.wait_for", side_effect=asyncio.TimeoutError),  # 模拟超时
        patch("asyncio.iscoroutinefunction", return_value=False),  # 使方法被视为非协程
        patch.object(BackgroundProcess, "add_error", MagicMock()) as add_error_mock,
    ):
        # 创建一个有超时设置的后台进程
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            process1 = BackgroundProcess(
                shell_cmd="echo test1", directory=temp_dir, description="Test process 1"
            )
            process1.pid = 10001
            ...
            process2 = BackgroundProcess(
                shell_cmd="echo test2", directory=temp_dir, description="Test process 2"
            )
            process2.pid = 10002
            process2.status = ProcessStatus.COMPLETED
            ...
            process3 = BackgroundProcess(
                shell_cmd="echo test3", directory=temp_dir, description="Test process 3"
            )
            process3.pid = 10003
