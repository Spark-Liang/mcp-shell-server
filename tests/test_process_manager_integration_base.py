    ...
    async def test_process_encoding(self):
        ...
        async with get_process_manager(self.create_manager) as process_manager:
            ...
            process_utf8 = await process_manager.create_process(
                shell_cmd=cmd, directory=os.getcwd(), encoding="utf-8"
            )

            stdout_utf8, stderr_utf8 = await process_manager.execute_with_timeout(
                process_utf8
            )
            ...
            if sys.platform == "win32":
                ...
                stdout_gbk, stderr_gbk = await process_manager.execute_with_timeout(
                    process_gbk
                )
