...
async def test_async_execute_basic():
    ...
    default_shell_executor.process_manager.start_process = AsyncMock(return_value="test-process-id")
...
async def test_async_execute_validation():
    ...
    with pytest.raises(ValueError, match="Directory must be an absolute path"):
        await default_shell_executor.async_execute(["echo", "hello"], directory="relative/path")
    
    # Non-existent directory
    with pytest.raises(ValueError, match="Directory does not exist"):
        await default_shell_executor.async_execute(["echo", "hello"], directory="/nonexistent/directory") 