...
class TestShellExecutorIntegration:
    ...
    def setup_method(self):
        ...
        if sys.platform == "win32":
            os.environ["ALLOW_COMMANDS"] = (
                f"echo,cmd.exe,ping,findstr,type,powershell.exe,python,{python_path}"
            )
        else:
            os.environ["ALLOW_COMMANDS"] = (
                f"echo,sh,bash,sleep,grep,cat,python,python3,{python_path}"
            )

        self.shell_executor = ShellExecutor()
        self.cmd_for_test_path = os.path.join(
            os.path.dirname(__file__), "cmd_for_test.py"
        )
    ...
    async def test_basic_execute(self):
        ...
        response = await self.shell_executor.execute(
            command=cmd, directory=self.test_dir
        )
    ...
    async def test_execute_with_timeout_success(self):
        ...
        response = await self.shell_executor.execute(
            command=cmd, directory=self.test_dir, timeout=5
        )
    ...
    async def test_execute_with_timeout_failure(self):
        ...
        response = await self.shell_executor.execute(
            command=cmd, directory=self.test_dir, timeout=1  # 设置较短的超时时间
        )

        # 验证是否因超时而失败
        assert response.error is not None, "Expected timeout error but got none"
        assert (
            "timed out" in response.error.lower() or "timeout" in response.error.lower()
        )
        assert response.status == -1

    @pytest.mark.asyncio
    @pytest.mark.skipif(
        sys.platform == "win32", reason="Windows平台上的管道命令测试暂时跳过"
    )
            ...
            if sys.platform == "win32":
                ...
                response = await self.shell_executor.execute(
                    command=[
                        "powershell.exe",
                        "-Command",
                        f"Get-Content {temp_path} | Select-String Test",
                    ],
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(
        sys.platform == "win32", reason="Windows平台上的编码测试暂时跳过"
    )
        ...
        if sys.platform == "win32":
            ...
            try:
                ...
                response_gbk = await self.shell_executor.execute(
                    command=["type", temp_path], directory=self.test_dir, encoding="gbk"
                )

                # 至少有一种编码应该能正确读取
                assert (
                    test_text in response_utf8.stdout
                    or test_text in response_gbk.stdout
                ), "中文字符无法正确读取"
            finally:
                # 清理临时文件
                if os.path.exists(temp_path):
            ...
            response = await self.shell_executor.execute(
                command=["echo", test_text], directory=self.test_dir, encoding="utf-8"
            )
    ...
    async def test_async_execute(self):
        ...
        assert any(
            "Async Test" in entry.text for entry in log_entries
        ), "输出中应该包含命令结果"
