...
def setup_mock_subprocess(monkeypatch):
    ...
    async def mock_create_subprocess_shell(
        ...
        if "echo" in cmd:
            # 直接返回固定的 "hello world" 输出，避免任何格式问题
            return MockProcess(stdout=b"hello world\n", stderr=b"", returncode=0)
        elif "pwd" in cmd:
            # 返回与 temp_test_dir 格式一致的路径
            if cwd:
                return MockProcess(stdout=cwd.encode() + b"\n", stderr=b"", returncode=0)
            return MockProcess(stdout=b".\n", stderr=b"", returncode=0)
        elif "cat" in cmd:
            return MockProcess(
        ...
        elif "chinese_output" in cmd:
            # 模拟中文输出（使用GBK/CP936编码）
            return MockProcess(stdout=b"\xd6\xd0\xce\xc4\xca\xe4\xb3\xf6", stderr=b"", returncode=0)
        else:
            return MockProcess(stdout=b"", stderr=b"", returncode=0)
...
async def test_tool_execution_timeout(monkeypatch, temp_test_dir):
    ...
    assert "encoding" in tool.inputSchema["properties"]
...
@pytest.mark.asyncio
async def test_call_tool_with_chinese_output(monkeypatch, temp_test_dir):
    """测试带有中文输出的命令执行，使用GBK/CP936编码"""
    setup_mock_subprocess(monkeypatch)
    monkeypatch.setenv("ALLOW_COMMANDS", "chinese_output")
    ...
    with pytest.raises(RuntimeError) as excinfo:
        await call_tool(
            "shell_execute",
            {
                "command": ["chinese_output"],
                "directory": temp_test_dir,
            },
        )
    assert "codec can't decode" in str(excinfo.value)
    
    # 使用正确的编码（GBK/CP936）应该成功
    result = await call_tool(
        "shell_execute",
        {
            "command": ["chinese_output"],
            "directory": temp_test_dir,
            "encoding": "gbk"  # 或 "cp936"
        },
    )
    assert len(result) == 1
    assert isinstance(result[0], TextContent)
    assert result[0].type == "text"
    assert result[0].text.strip() == "中文输出"


@pytest.mark.asyncio
async def test_call_tool_with_different_encodings(monkeypatch, temp_test_dir):
    """测试不同编码选项的效果"""
    setup_mock_subprocess(monkeypatch)
    monkeypatch.setenv("ALLOW_COMMANDS", "chinese_output")
    
    # 使用CP936编码（Windows简体中文）
    result = await call_tool(
        "shell_execute",
        {
            "command": ["chinese_output"],
...
@pytest.mark.asyncio
async def test_call_tool_with_default_encoding(monkeypatch, temp_test_dir):
    """测试默认编码设置处理"""
    setup_mock_subprocess(monkeypatch)
    monkeypatch.setenv("ALLOW_COMMANDS", "chinese_output")
    
    # 1. 使用环境变量设置的默认编码
    monkeypatch.setenv("DEFAULT_ENCODING", "gbk")
    result = await call_tool(
        "shell_execute",
        {
            "command": ["chinese_output"],
            "directory": temp_test_dir,
            # 不指定encoding，应该使用环境变量DEFAULT_ENCODING
        },
    )
    assert len(result) == 1
    assert result[0].text.strip() == "中文输出"
    ...
    # 2. 清除环境变量，使用系统默认编码
    monkeypatch.delenv("DEFAULT_ENCODING", raising=False)
    ...
    # 3. 显式指定的编码应该覆盖环境变量
    monkeypatch.setenv("DEFAULT_ENCODING", "utf-8")  # 错误的编码
    result = await call_tool(
        "shell_execute",
        {
            "command": ["chinese_output"],
            "directory": temp_test_dir,
            "encoding": "gbk"  # 显式指定正确的编码
        },
    )
    assert len(result) == 1
    assert result[0].text.strip() == "中文输出"
